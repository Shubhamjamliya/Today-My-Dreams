...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!res.success) setErr(res.message || 'Login failed');
    setLoading(false);
  };
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-[#FCD24C] to-[#F59E0B] px-8 py-6 text-center">
            ...
            <p className="text-gray-800 text-sm mt-1">Access your vendor panel</p>
          </div>
          <div className="px-8 py-8">
            ...
            <div className="text-center mt-6">
              <p className="text-sm text-gray-500">
                New vendor? <Link to="/vendor/register" className="text-gray-900 font-semibold underline">Register</Link>
              </p>
            </div>
          </div>
        </div>
        <div className="text-center mt-6">
          <p className="text-sm text-gray-500">Secure vendor access</p>
        </div>
      </div>
    </div>
  );
