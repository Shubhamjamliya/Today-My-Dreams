...
const ServiceHero = memo(({ selectedCategories }) => {
  return (
      ...
      <div className="container mx-auto px-6 relative z-10 w-full">
        ...
        >
          ...
          <h1 className="text-3xl md:text-5xl font-black text-white mb-2 tracking-tight leading-tight">
            {selectedCategories.main || "Exquisite Collections"}
          </h1>
          <p className="text-slate-300 text-base md:text-lg font-medium max-w-xl">
            Professional decoration services for your most memorable moments.
          </p>
        </motion.div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-3xl lg:sticky lg:top-[112px] lg:h-[calc(100vh-140px)] flex flex-col z-30 transition-all duration-300 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-900/5 hover:ring-slate-900/10 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
      <div className="p-5 pb-4 border-b border-slate-100 bg-white/50 backdrop-blur-xl rounded-t-3xl z-10 shrink-0">
        <div className="flex items-center gap-3 px-1">
          ...
          <div>
            <h3 className="font-black text-slate-800 tracking-tight text-lg leading-tight">Categories</h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Service</p>
          </div>
        </div>
      </div>
...
const TopFilterSection = memo(({
  ...
  // Close price popover when clicking outside (simple implementation using backdrop)
...
const Service = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Determine which category and products endpoint to use
        const isShop = moduleQuery === 'shop';
        let productsUrl = isShop ? config.API_URLS.SHOP : config.API_URLS.PRODUCTS;
        ...
        if (!productsResponse.ok) throw new Error('Failed to fetch services');

        const productsData = await productsResponse.json();
        const productsArray = (productsData.products || []).map(p => ({
          ...p,
          module: isShop ? 'shop' : 'service'
        }));
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <SEO
        title="Our Services | TodayMyDream"
        description="Explore our range of decoration services for birthdays, weddings, and more."
        url="https://todaymydream.com/services"
      />
            ...
            <div className="z-10 bg-[#F8FAFC] py-4 flex flex-col md:flex-row justify-between items-start md:items-center mb-2 gap-4 border-b border-slate-200/60 -mx-4 px-4 md:-mx-0 md:px-0 transition-all duration-300">
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                  {selectedCategories.sub || selectedCategories.main || "Exquisite Decoration Services"}
                </h2>
              ...
              >
                <FunnelIcon className="w-4 h-4" /> Categorize Services
              </button>
            </div>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-8 w-full">
                  <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl flex justify-between items-center w-full">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-slate-700">Finding services for <span className="font-bold text-slate-900">"{searchQuery}"</span></p>
                    </div>
            ...
            <div className="h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar scroll-smooth border border-slate-200/80 bg-white/50 rounded-3xl shadow-sm p-4 md:p-6 relative">
              {loading ? (
                ...
                <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-red-200 w-full">
                  <p className="text-red-500 font-bold mb-2">Service unavailable</p>
                ...
                <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-slate-200 w-full">
                  ...
                  <p className="text-slate-400 max-w-xs mx-auto mb-8 leading-relaxed">Try adjusting your filters to find the perfect decoration service.</p>
          ...
          <>
            ...
            >
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h2 className="text-lg font-black text-slate-900">Service Options</h2>
