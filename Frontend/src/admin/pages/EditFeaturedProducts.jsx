...
const EditFeaturedProducts = () => {
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      const fetchProduct = async () => {
        try {
          // First try to get from featured products
          const response = await apiService.getFeaturedProduct(id);
          let prod = response.data.product;

          // If not found in featured products, try other collections
          if (!prod) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!product.name || !product.price || !product.regularPrice || !product.category || !product.description) {
      ...
      if (files.image5) formData.append('image5', files.image5);
      if (files.image6) formData.append('image6', files.image6);
      if (files.image7) formData.append('image7', files.image7);
      if (files.image8) formData.append('image8', files.image8);
      if (files.image9) formData.append('image9', files.image9);

      if (isNew) {
        await apiService.createFeaturedProduct(formData);
        showToast("Product created successfully!");
      } else {
        if (Object.values(files).some(file => file instanceof File)) {
          await apiService.updateFeaturedProduct(product.id, formData);
        } else {
          const productData = {
            ...product,
          ...
          await apiService.updateFeaturedProduct(product.id, productData);
        }
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="p-6 sm:p-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-8">
              {isNew ? "Add New Featured Product" : "Edit Featured Product"}
            </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    <label className="block font-medium text-gray-700">Care Instructions (Old Field)</label>
                    <input
                      type="text"
                      name="care"
                      value={product.care}
                  ...
                  {/* Included Items */}
                  <div>
                    <label className="block font-medium text-gray-700 mb-2">What's Included ✓</label>
                    <div className="space-y-2">
                      {product.included.map((item, index) => (
                        <div key={index} className="flex gap-2">
                          <input
                            ...
                            value={item}
                            onChange={(e) => {
                              const newIncluded = [...product.included];
                              newIncluded[index] = e.target.value;
                              setProduct({ ...product, included: newIncluded });
                            }}
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const newIncluded = product.included.filter((_, i) => i !== index);
                              setProduct({ ...product, included: newIncluded });
                            }}
                      ...
                      <button
                        type="button"
                        onClick={() => setProduct({ ...product, included: [...product.included, ''] })}
