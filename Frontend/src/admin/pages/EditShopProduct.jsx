...
const EditShopProduct = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await apiService.getCategories({ module: 'shop' }); // Pass module: 'shop'
  ...
  useEffect(() => {
    if (!isNew) {
      setLoading(true);
      apiService.getProduct(id, { module: 'shop' })
        .then((response) => {
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-center mb-8">
              ...
              <h1 className="text-3xl font-bold text-gray-900">
                {isNew ? "Add New Shop Product" : "Edit Shop Product"}
              </h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-6">Basic Information (Shop)</h2>
              ...
              <div className="flex items-center justify-end space-x-4">
                <button type="button" onClick={() => navigate("/admin/shop/products")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center space-x-2">
                  ...
                  <span>{isNew ? "Create Shop Product" : "Update Shop Product"}</span>
                </button>
              </div>
            </form>
          </div>
